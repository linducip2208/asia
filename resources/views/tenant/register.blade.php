...
<body class="h-full flex items-center justify-center py-12 px-4">
    <div class="w-full max-w-lg">
        ...
        @endif
        <form method="POST" action="{{ route('tenant.register') }}" class="bg-white shadow rounded-xl p-8 space-y-5">
            @csrf
            <div>
            ...
            <div>
                ...
                <select name="plan_slug" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 px-4 py-2.5 border">
                    @foreach($plans as $plan)
                        <option value="{{ $plan->slug }}" @if($plan->slug === 'free') selected @endif>{{ $plan->name }} — {{ $plan->formattedPriceMonthly() }}/bulan</option>
                    @endforeach
                </select>
            </div>
