        ...
        <form wire:submit="save" class="space-y-6">
            <div class="fi-section rounded-xl bg-white shadow-sm ring-1 ring-gray-950/5 dark:bg-gray-900 dark:ring-white/10">
                <div class="fi-section-header px-6 py-4">
                    <h3 class="text-base font-semibold leading-6 text-gray-950 dark:text-white">{{ $editId ? 'Edit Webhook' : 'Tambah Webhook' }}</h3>
                </div>
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody>
                            ...
                            <tr class="border-b border-gray-100 dark:border-gray-800">
                                <td class="px-3 py-3">{{ $webhook['name'] }}</td>
                                <td class="px-3 py-3"><span class="text-xs font-mono bg-gray-100 dark:bg-gray-800 px-2 py-0.5 rounded">{{ $webhook['api_format'] }}</span></td>
                                <td class="px-3 py-3 text-xs text-gray-500 max-w-xs truncate">{{ $webhook['base_url'] }}</td>
                                <td class="px-3 py-3">
                                    <span class="text-xs px-2 py-0.5 rounded-full font-medium {{ $webhook['is_active'] ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500' }}">
                                        {{ $webhook['is_active'] ? 'Aktif' : 'Nonaktif' }}
                                    </span>
                                </td>
                                <td class="px-3 py-3">
                                    <div class="flex gap-1">
                                        <button type="button" wire:click="edit({{ $webhook['id'] }})" class="text-primary-600 hover:text-primary-800 text-xs underline">Edit</button>
                                        <button type="button" wire:click="delete({{ $webhook['id'] }})" wire:confirm="Hapus webhook ini?" class="text-red-600 hover:text-red-800 text-xs underline">Hapus</button>
                                    </div>
                                </td>
                            </tr>
                            @endforeach
